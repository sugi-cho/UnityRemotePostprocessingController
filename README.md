...
- 複数PCの Unity への State 同期（Syncビューで同期先を管理）
...
3. Play 実行
4. ブラウザで `http://localhost:8080/` を開く
5. 初回アクセス時は WebUI でパスワードを設定
6. 2回目以降はログイン後に `Override ON` / `Volume All` を切り替えて調整
7. Preset を `Save` して次回起動時に自動ロードを確認
...
## 複数PC同期（Syncビュー）
- ヘッダーの `Sync` ビューで同期先を追加・有効/無効・解除できます
- `Sync target IP or URL` に `192.168.1.20:8080` などを入力して `Add`
- 同期先が認証有効の場合は `Target password` も入力すると自動ログインして同期可能
- `Override ON` / `Volume All` で調整した `PATCH /state` は、同期先へ同時転送されます
- 同期先が未起動・切断時は `Disconnected` とエラーで表示されます

注意:
- 同期先への再ログインのため、`Target password` は `syncTargets.json` に保存されます（LAN 内運用前提）
- 転送パッチには `X-URP-Remote-Relay: 1` が付き、再転送ループを防止します
...
## 認証
- 初回アクセス時（`auth.json` 不在時）に WebUI でパスワードを設定
- サーバーには平文パスワードを保存せず、ハッシュ化情報のみ保存
- ログイン成功時にセッショントークンを発行（短命）
- `auth.json` を削除すると、次回アクセス時に再セットアップ可能

### 認証情報保存先
`Application.persistentDataPath/RemotePostprocess/auth.json`

Windows 例:
`C:\Users\<User>\AppData\LocalLow\DefaultCompany\UnityRemotePostprocessingController\RemotePostprocess\auth.json`

### 同期設定保存先
`Application.persistentDataPath/RemotePostprocess/syncTargets.json`

Windows 例:
`C:\Users\<User>\AppData\LocalLow\DefaultCompany\UnityRemotePostprocessingController\RemotePostprocess\syncTargets.json`
...
- `POST /presets/delete`
- `GET /sync/targets`
- `POST /sync/targets/add`
- `POST /sync/targets/remove`
- `POST /sync/targets/set-enabled`
- `GET /ws`（WebSocket）
- `POST /auth/logout`
...
- パスワードを忘れた/再設定したい:
  - `RemotePostprocess/auth.json` を削除してから Play を再起動
